...
def _lorimer2015_msp_periods():
    """Lorimer et al. (2015)
    """
    #print "Lorimer et al. (2015)\n"
    #def cal_cdf (x, y):
    #    cdf = []
    #    for x_temp in x:
    #        mask = x <= x_temp
    #        cdf.append(sum(y[mask]))
    #        cdf = cdf/sum(y)
    #        
    #    return cdf
    ...
    y = np.exp(-(np.log(x) - E)**2.0/(2.0*F**2.0))*(1.0/x)
    ...
    cdf_func = interp1d(cdf, x)

    x_rand = np.random.rand(1)[0]

    # return period in millisecond
    return cdf_func(x_rand)
