...
void myprint(void);
double run_ymw16(double gl, double gb, double dist);
...
{
  double ne0, ne, ne1, ne2, ne3, ne4, ne5, ne6, ne7, ne8, ne9, ne10;
  ne9 = 0.0;  // need to check
  double dist, xx, yy, zz, r, sl, cl, sb, cb, ll, hh;
  //double gl, gb, dordm, dist, xx, yy, zz, r, sl, cl, sb, cb, ll, hh;
  double nstep, dstep, dmstep;
  //static double dd, dtest, dmpsr, rr;
  double dd, dtest, dmpsr, rr;
  double dmm=0;
  double dm=0;
  double DM_MC=0;
  double DM_Gal=0;
  ...
  strcpy(dirname,"/Users/dai02a/Soft/psrpop/PsrPopPy_test/lib/ymw16/");
  if(!strcmp(dirname,"NULL")){
    if(getenv("YMW16_DIR")==NULL){
      printf("Warning: YMW16_DIR set to local directory\n");
      strcpy(dirname,"/Users/dai02a/Soft/psrpop/PsrPopPy_test/lib/ymw16/");
  ...
  if(vbs>=1)printf("File directory: %s %d\n",dirname,np);
